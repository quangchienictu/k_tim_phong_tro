...
class UserController extends Controller
{
    ...
    public function post_login(Request $req)
    {
        ...
        if (Auth::attempt(['username' => $req->txtuser, 'password' => $req->txtpass, 'tinhtrang' => 1])) {
            return redirect('/');
        } else
    ...
    {
        $categories = Categories::all();
        $tinDangs = Motelroom::select('motelrooms.*', 'categories.name')
                    ->leftJoin('categories','motelrooms.category_id', 'categories.id')
                    ->where('user_id', Auth::id())->get();
        return view('home.tindadang', [
            'categories' => $categories,
            'tinDangs' => $tinDangs
        ]);
    }
    ...
    public function comment(Request $res){
         $rating = new rating;
        $rating->id_user = $res->user;
        $rating->id_motel = $res->id_post;
        $rating->comment = $res->data;
        $rating->rate = $res->value;
        $rating->status = 0;
        $rating->save();
        echo "ok";

    }
    public function edit_dangtin($id)
    {
        $district = District::all();
        $categories = Categories::all();
        $item = Motelroom::find($id);
    ...
    public function edittindadang($id){
        $item = Motelroom::find($id);
       if($item->status==0){
         $item->status = 1;
         ...
         $item->status = 0;
           ...
           $item->save();
         return redirect()->route('user.tindadang');
    }
    /* Đăng tin */
    public function edit_dangtin_post(Request $request, $id)
    {
        $request->validate([
            'txttitle' => 'required',
            'txtaddress' => 'required',
        ...
        /* New Phòng trọ */
//        dd($request->all(), $id);
        $motel = Motelroom::findBySlug($request->id);
//        dd($motel);
        $motel->title = $request->txttitle;
        $motel->description = $request->txtdescription;
        $motel->price = $request->txtprice;
        ...
        $motel->save();
        return redirect()->route('user.tindadang')->with('success', 'Đăng tin thành công. Vui lòng đợi Admin kiểm duyệt');
    }
}
