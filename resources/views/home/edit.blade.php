...
@section('content')
                ...
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        ...
                        @if(Auth::user()->tinhtrang != 0)
                            <form action="{{ route ('user.editdangtinpost', $item->id) }}" method="POST" enctype="multipart/form-data">
                                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                <input type="hidden" name="id" value="{{$item->slug}}">
                                <div class="form-group">
                                    <label for="usr">Tiêu đề bài đăng:</label>
                                    <input type="text" class="form-control" name="txttitle"
                                           value="{{old('txttitle', $item->title)}}">
                                </div>
                                <div class="form-group">
                                    ...
                                    <input type="text" id="location-text-box" name="txtaddress" class="form-control"
                                           value="{{old('txttitle', $item->address)}}"/>
                                    ...
                                    <input type="hidden" id="txtaddress" name="txtaddress" value="{{old('txttitle', $item->address)}}"
                                           class="form-control"/>
                                    <input type="hidden" id="txtlat" value="{{old('txtlat', json_decode($item->latlng,true)[0])}}" name="txtlat" class="form-control"/>
                                    <input type="hidden" id="txtlng" value="" name="{{old('txtlng', json_decode($item->latlng,true)[1])}}" class="form-control"/>
                                </div>
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="number" name="txtprice" class="form-control" value="{{old('txtprice', $item->price)}}"
                                                   placeholder="750000">
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="number" name="txtarea" class="form-control" placeholder="16" value="{{old('txtarea', $item->area)}}">
                                        </div>
                                    </div>
                                </div>
                                <div class="row">
                                            ...
                                            <select class="selectpicker pull-right" data-live-search="true"
                                                ...
                                                @foreach($district as $quan)
                                                    <option data-tokens="{{$quan->slug}}"
                                                            value="{{ $quan->id }}" {{$quan->id == old('iddistrict', $item->district_id) ? 'selected' : ''}}>{{ $quan->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                            ...
                                            <select class="selectpicker pull-right" data-live-search="true"
                                                ...
                                                @foreach($categories as $category)
                                                    <option data-tokens="{{$category->slug}}"
                                                            value="{{ $category->id }}" {{$category->id == old('idcategory', $item->category_id) ? 'selected' : ''}}>{{ $category->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="txtphone" class="form-control" value="{{old('txtphone', $item->phone)}}"
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control" rows="5" name="txtdescription"
                                              style=" resize: none;">{{old('txtdescription', $item->description)}}</textarea>
                                </div>
                                <div class="form-group">
                                ...
                                <button class="btn btn-primary">Cập Nhật Tin</button>
                            </form>
                        @else
                            <div class="alert bg-danger">
                                <button type="button" class="close" data-dismiss="alert"><span>×</span><span
    ...
    <script>
        ...
        function initialize() {
            var mapOptions = {
                center: {lat: 21.033791, lng: 105.805891},
                zoom: 12
            };
            ...
            if (navigator.geolocation) {
                navigator.geolocation.getCurrentPosition(function (position) {
                    var pos = new google.maps.LatLng({{json_decode($item->latlng,true)[0]}},
                        {{json_decode($item->latlng,true)[1]}});
                    console.log(pos.lat);
            ...
            function handleNoGeolocation(errorFlag) {
                ...
                var options = {
                    map: map,
                    zoom: 19,
                    position: new google.maps.LatLng(21.033791, 105.805891),
                    content: content
                };
